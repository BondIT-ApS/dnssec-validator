...
class TLSAValidator:
    ...
    def validate_tlsa(self, port=443, protocol='tcp', timeout=10):
        ...
        try:
            ...
            if not tlsa_records:
                ...
                result['warnings'].append(f'No TLSA records found for _{port}._{protocol}.{self.domain}')
                return result
            ...
            try:
                ...
                validation_results = self._validate_dane_associations(tlsa_records, cert_info)
        ...
        try:
            ...
            for rr in answer.rrset:
                tlsa_record = {
                    'name': tlsa_name,
                    'usage': rr.usage,
                    'usage_description': self.cert_usage_types.get(rr.usage, f'Unknown ({rr.usage})'),
                    'selector': rr.selector,
                    'selector_description': self.selector_types.get(rr.selector, f'Unknown ({rr.selector})'),
                    'mtype': rr.mtype,
                    'mtype_description': self.matching_types.get(rr.mtype, f'Unknown ({rr.mtype})'),
            ...
            logging.info(f"TLSA query returned no answer for _{port}._{protocol}.{self.domain}")
            return []
        except Exception as e:
    ...
    def _validate_dane_associations(self, tlsa_records, cert_info):
        ...
        for tlsa_record in tlsa_records:
            association_result = self._validate_single_association(tlsa_record, cert_info)
    ...
    def _validate_single_association(self, tlsa_record, cert_info):
        ...
        try:
            ...
            if tlsa_record['selector'] == 0:  # Full certificate
                ...
                result['reason'] = f"Unsupported selector type: {tlsa_record['selector']}"
                return result
            ...
            result['match_details']['hash_algorithm'] = self.matching_types.get(tlsa_record['mtype'], 'Unknown')
    ...
    def _analyze_tlsa_records(self, result):
        ...
        for tlsa_record in result['tlsa_records']:
            record_analysis = {
                ...
                'selector_analysis': self._analyze_selector_type(tlsa_record['selector']),
            ...
            if tlsa_record['usage'] in [0, 1]:  # PKIX
                record_analysis['security_notes'].append("Uses PKIX validation - requires valid CA chain")
            elif tlsa_record['usage'] in [2, 3]:  # DANE
                record_analysis['security_notes'].append("Uses DANE validation - bypasses traditional CA validation")
            
            if tlsa_record['mtype'] == 0:
                record_analysis['security_notes'].append("Full data matching - larger DNS records but precise matching")
            elif tlsa_record['mtype'] in [1, 2]:
                record_analysis['security_notes'].append("Hash-based matching - smaller DNS records, cryptographically secure")
    ...
    def _analyze_usage_type(self, usage):
        ...
        if usage == 0:  # PKIX-TA
            analysis['description'] = 'CA constraint - certificate must validate via traditional PKIX and match the association'
            analysis['security_implications'] = 'Provides additional security on top of traditional CA validation'
        elif usage == 1:  # PKIX-EE
            analysis['description'] = 'Service certificate constraint - end entity cert must validate via PKIX and match'
            analysis['security_implications'] = 'Pins specific certificate while maintaining CA validation'
        elif usage == 2:  # DANE-TA
            analysis['description'] = 'Trust anchor assertion - certificate must chain to the specified trust anchor'
            analysis['security_implications'] = 'Bypasses traditional CAs, uses DNS-specified trust anchor'
            analysis['recommended'] = False  # More complex to implement correctly
        elif usage == 3:  # DANE-EE
            analysis['description'] = 'Domain-issued certificate - certificate must match exactly'
            analysis['security_implications'] = 'Complete bypass of CA system, DNS is the only trust source'
            analysis['recommended'] = True  # Simple and effective
        else:
            analysis['recommended'] = False
            analysis['security_implications'] = 'Unknown usage type - may not be supported by validators'
    ...
    def _analyze_selector_type(self, selector):
        ...
        if selector == 0:  # Full certificate
            ...
            analysis['advantages'] = ['Precise matching', 'No ambiguity about which certificate']
            analysis['disadvantages'] = ['Larger DNS records', 'Must update DNS when certificate changes']
        elif selector == 1:  # SPKI
            analysis['description'] = 'Matches against the Subject Public Key Info'
            analysis['advantages'] = ['Smaller DNS records', 'Can survive certificate renewal with same key']
    ...
    def _assess_security(self, result):
        ...
        if result['tlsa_status'] == 'valid':
            ...
            assessment['weaknesses'].append('No TLSA records found - DANE not implemented')
        
        # Check for modern hash algorithms
        for record in result['tlsa_records']:
            if record['mtype'] in [1, 2]:  # SHA-256 or SHA-512
                score += 10
                assessment['strengths'].append(f"Uses cryptographically strong hashing: {record['mtype_description']}")
            elif record['mtype'] == 0:
                score += 5
                assessment['strengths'].append("Uses full certificate matching - high precision")
        
        # Check for DANE-EE usage (most secure)
        for record in result['tlsa_records']:
            if record['usage'] == 3:  # DANE-EE
                score += 15
                assessment['strengths'].append("Uses DANE-EE - bypasses CA system completely")
        ...
        if result['certificate_info']:
            ...
            if validity.get('currently_valid'):
                ...
                if days_remaining > 30:
                    ...
                    assessment['risk_factors'].append(f'Certificate expires in {days_remaining} days')
                else:
                    score -= 20
    ...
    def _generate_tlsa_troubleshooting(self, result):
        ...
        for invalid_assoc in result['dane_validation'].get('invalid_associations', []):
            ...
            troubleshooting.append(f"❌ TLSA record (usage:{tlsa['usage']}, selector:{tlsa['selector']}, type:{tlsa['mtype']}): {invalid_assoc['reason']}")
    ...
    def _generate_tlsa_recommendations(self, result):
        ...
        if result['tlsa_status'] == 'valid':
            recommendations.append("✅ DANE validation successful - excellent security posture")
        ...
        if modern_hash:
            recommendations.append("✅ Modern hash algorithms (SHA-256/SHA-512) detected")
        else:
            recommendations.append("🔄 Consider using SHA-256 or SHA-512 for better security")
        
        # Certificate management
        if result['certificate_info']:
            cert_analysis = result['detailed_analysis'].get('certificate_analysis', {})
            days_remaining = cert_analysis.get('validity_period', {}).get('days_remaining', 0)
            
            if days_remaining < 30:
                recommendations.append("⏰ Plan certificate renewal and TLSA record updates")
