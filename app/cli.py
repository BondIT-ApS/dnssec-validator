...
def create_app():
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = os.getenv(
        "DATABASE_URL", "sqlite:///dnssec_validator.db"
    )
...
def cleanup_logs(days, dry_run):
    ...
    with app.app_context():
        try:
            ...
            if dry_run:
                click.echo(
                    f"DRY RUN: Would delete {count} log entries older than {days} days (before {cutoff_date})"
                )
                return
            ...
            click.echo(
                f"Successfully deleted {deleted} log entries older than {days} days"
            )
...
def recent_requests(hours):
    ...
    with app.app_context():
        try:
            ...
            recent = (
                RequestLog.query.filter(RequestLog.timestamp >= since)
            ...
            click.echo(
                "Timestamp                Domain                  IP              Status  Source"
            )
            click.echo("-" * 80)
