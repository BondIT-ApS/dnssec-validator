...
limiter = Limiter(
    ...
    app=app,
    default_limits=[
        f"{rate_limits['global_day']} per day",
...
logger.info(
    f"DNSSEC Validator starting with log level: {logging.getLevelName(log_level)}"
)
...
def log_request(response):
    ...
    try:
        ...
        client = (
            "webapp"
            if request.headers.get("X-Client", "").lower() == "webapp"
            else "external"
        )
...
dnskey_record_model = api.model(
    ...
    {
        "zone": fields.String(
            required=True, description="The zone this key belongs to"
        ),
        "flags": fields.Integer(required=True, description="DNSKEY flags"),
        "protocol": fields.Integer(
            required=True, description="Protocol (always 3 for DNSSEC)"
        ),
        "algorithm": fields.Integer(
            required=True, description="Cryptographic algorithm"
        ),
        "key_tag": fields.Integer(required=True, description="Key identifier"),
    },
)

ds_record_model = api.model(
    "DSRecord",
    {
        "zone": fields.String(
            required=True, description="The zone this DS record belongs to"
        ),
...
rrsig_record_model = api.model(
    "RRSIGRecord",
    {
        "type_covered": fields.String(
            required=True, description="Record type covered by this signature"
        ),
        "algorithm": fields.Integer(
            required=True, description="Cryptographic algorithm"
        ),
        "labels": fields.Integer(
            required=True, description="Number of labels in the signed name"
        ),
        "original_ttl": fields.Integer(
            required=True, description="Original TTL of the signed RRset"
        ),
        "expiration": fields.Integer(
            required=True, description="Signature expiration time"
        ),
        "inception": fields.Integer(
            required=True, description="Signature inception time"
        ),
        "key_tag": fields.Integer(
            required=True, description="Key tag of the signing key"
        ),
        "signer": fields.String(required=True, description="Name of the signing zone"),
    },
)

records_model = api.model(
    "DNSSECRecords",
    {
        "dnskey": fields.List(
            fields.Nested(dnskey_record_model), description="DNSKEY records"
        ),
        "ds": fields.List(fields.Nested(ds_record_model), description="DS records"),
        "rrsig": fields.List(
            fields.Nested(rrsig_record_model), description="RRSIG records"
        ),
    },
)

chain_of_trust_model = api.model(
    "ChainOfTrust",
    {
        "zone": fields.String(required=True, description="Zone name"),
        "status": fields.String(
            required=True, description="Validation status for this zone"
        ),
...
tlsa_summary_model = api.model(
    "TLSASummary",
    {
        "status": fields.String(
            required=True,
            description="TLSA validation status",
            enum=["valid", "invalid", "no_records", "cert_unavailable", "error"],
        ),
        "records_found": fields.Integer(
            required=True, description="Number of TLSA records found"
        ),
        "dane_status": fields.String(
            required=True, description="DANE validation status"
        ),
...
validation_result_model = api.model(
    "ValidationResult",
    {
        ...
        "status": fields.String(
            ...
            enum=["valid", "invalid", "insecure", "error"],
        ),
        "validation_time": fields.String(
            required=True, description="ISO timestamp of validation"
        ),
        "chain_of_trust": fields.List(
            fields.Nested(chain_of_trust_model),
            description="Chain of trust validation details",
        ),
...
error_model = api.model(
    ...
    {
        "domain": fields.String(
            required=True, description="The domain that caused the error"
        ),
        "status": fields.String(
...
timeseries_model = api.model(
    "TimeSeriesData",
    {
        "data": fields.List(
            fields.Nested(hourly_data_model), description="Hourly request data"
        ),
...
class DNSSECValidation(Resource):
    ...
    @ns_validate.response(
        200, "Success - Domain validation completed", validation_result_model
    )
        ...
        try:
            ...
            if not extracted_domain or not is_valid_domain_format(extracted_domain):
                return {
                    ...
                    "status": "error",
                    "errors": [
                        f"Invalid domain format or unable to extract domain from: {original_input}"
                    ],
                }, 400
            ...
            logger.error(
                f"DNSSEC validation failed for domain {domain}: {str(e)}", exc_info=True
            )
            return {
                "domain": domain,
                "status": "error",
...
class DNSSECDetailedValidation(Resource):
    ...
    def get(self, domain):
        ...
        try:
            ...
            logger.error(
                f"Detailed DNSSEC analysis failed for domain {domain}: {str(e)}",
                exc_info=True,
            )
            return {
                "domain": domain,
...
class AnalyticsOverview(Resource):
    ...
    @ns_analytics.response(
        200, "Success - Analytics overview data", analytics_overview_model
    )
    @limiter.limit(
        f"{rate_limits['api_minute']} per minute; {rate_limits['api_hour']} per hour"
    )
    def get(self):
        ...
        try:
            ...
            logger.debug(
                f"Analytics overview requested for period: {period} (hours={hours}, days={days})"
            )
            ...
            validation_ratio = RequestLog.get_external_validation_ratio(
                days=days, hours=hours
            )
            top_domains = RequestLog.get_external_top_domains(
                limit=10, days=days, hours=hours
            )
...
class AnalyticsTimeSeries(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            hourly_data = RequestLog.get_external_hourly_requests(
                hours=hours, window_every=window
            )

            # Format data for chart
            chart_data = [
                {"timestamp": timestamp, "requests": count}
                for timestamp, count in hourly_data
            ]

            total_requests = sum(count for _, count in hourly_data)

            return {"data": chart_data, "period": period, "total": total_requests}, 200
...
class AnalyticsSources(Resource):
    ...
    @ns_analytics.param(
        "period", "Time period: 1h, 24h, 7d, 30d", _in="query", default="7d"
    )
...
def ratelimit_handler(e):
    ...
    if request.path.startswith("/api/"):
        ...
        response = render_template(
            ...
            limit=str(e.description),
            retry_after=int(retry_after),
            reset_time=datetime.fromtimestamp(reset_time, tz=timezone.utc).strftime(
                "%H:%M:%S UTC"
            ),
        )
...
@app.route("/")
@limiter.limit(
    f"{rate_limits['web_minute']} per minute; {rate_limits['web_hour']} per hour"
)
def index():
